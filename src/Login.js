...
function Login() {
  ...
  async function handleSubmit(e) {
    ...
    if (isReset) {
      var { error: resetErr } = await supabase.auth.resetPasswordForEmail(emailLower);
      if (resetErr) { setError(resetErr.message); }
      else { setSuccess("Te hemos enviado un email para restablecer tu contraseña. Revisa tu bandeja (y spam)."); }
      setLoading(false);
      return;
    }
    ...
    <div style={{ minHeight:"100vh", background:"#0e0e1a", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans',sans-serif", padding:20 }}>
      ...
      <div style={{ width:"100%", maxWidth:400 }}>
        ...
        <div style={{ background:"linear-gradient(145deg,#1a1a2e,#16213e)", border:"1px solid #2a2a4a", borderRadius:20, padding:"32px 28px" }}>
          <h2 style={{ margin:"0 0 24px", fontSize:20, color:"#e0e0ff", textAlign:"center" }}>{isReset ? "Recuperar contraseña" : isRegister ? "Crear cuenta" : "Iniciar sesión"}</h2>
          ...
          <button onClick={handleSubmit} disabled={loading || !email || (!isReset && password.length < 6)} style={{ width:"100%", padding:"14px", borderRadius:12, border:"none", background:loading?"#3a3a5a":"linear-gradient(135deg,#6c5ce7,#a66efa)", color:"#fff", fontSize:15, fontWeight:600, cursor:loading?"default":"pointer", opacity:(!email||(!isReset&&password.length<6))?0.5:1 }}>{loading?"Cargando...":isReset?"Enviar email de recuperación":isRegister?"Crear cuenta":"Entrar"}</button>
          <div style={{ textAlign:"center", marginTop:16, display:"flex", flexDirection:"column", gap:8 }}>
            {!isReset && (
              <button onClick={function(){setIsReset(true);setError("");setSuccess("");}} style={{ background:"none", border:"none", color:"#e74c3c", fontSize:12, cursor:"pointer" }}>¿Olvidaste tu contraseña?</button>
            )}
            <button onClick={function(){setIsRegister(!isRegister);setIsReset(false);setError("");setSuccess("");}} style={{ background:"none", border:"none", color:"#a66efa", fontSize:13, cursor:"pointer" }}>{isRegister?"¿Ya tienes cuenta? Inicia sesión":isReset?"Volver a iniciar sesión":"¿Primera vez? Crear cuenta"}</button>
          </div>
        </div>
